...
class SMPLTemplate(nn.Module):
    ...
    def remove_instance(self, ins_id):
        # Keep all per-human buffers in sync to avoid mask/index shape mismatches.
        device = self.J_canonical.device
        ins_mask = torch.ones(self.num_human, dtype=torch.bool, device=device)
        ins_mask[ins_id] = False
        self.J_canonical = self.J_canonical[ins_mask]
        self.W = self.W[ins_mask]
        self.A0_inv = self.A0_inv[ins_mask]
        self.j0_t = self.j0_t[ins_mask]
        self.init_beta = self.init_beta[ins_mask]
        self.num_human = int(self.J_canonical.shape[0])
