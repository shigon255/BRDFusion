...
try:
    ...
    print("Warning: gsplat.rendering.rasterization_2dgs not found. Some functionalities may be limited.")
    # define a dummy function to avoid errors
    def rasterization_2dgs(*args, **kwargs):
        raise NotImplementedError("rasterization_2dgs is not available. Please ensure gsplat.rendering module is properly installed.")
...
class dataclass_gs:
    ...
    def features_dc(self):
        if "features_dc" in self.detach_keys:
            return self._features_dc.detach()
        else:
            return self._features_dc
    
    @property
    def features_rest(self):
        if "features_rest" in self.detach_keys:
            return self._features_rest.detach()
        else:
        ...
        for group_idx, group in enumerate(optimizer.param_groups):
            ...
            if name in param_dict.keys():
                ...
                if "exp_avg" in param_state:
                    ...
                    print(f"Warning: 'exp_avg' not found in param_state of {name} when removing from optimizer")
                    
                if "exp_avg_sq" in param_state:
                    param_state["exp_avg_sq"] = param_state["exp_avg_sq"][~deleted_mask]
                else:
                    print(f"Warning: 'exp_avg_sq' not found in param_state of {name} when removing from optimizer")
        ...
        for group_idx, group in enumerate(optimizer.param_groups):
            ...
            if name in param_dict.keys():
                ...
                if "exp_avg" in param_state and "exp_avg_sq" in param_state:
                    ...
                    print(f"Warning: 'exp_avg' or 'exp_avg_sq' not found in param_state of {name} when duplicating in optimizer")
                del optimizer.state[old_params]
                optimizer.state[new_params[0]] = param_state
                optimizer.param_groups[group_idx]["params"] = new_params
                del old_params
