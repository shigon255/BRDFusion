...
class RigidNodes(VanillaGaussians):
    ...
    def create_from_pcd(self, instance_pts_dict: Dict[str, torch.Tensor]) -> None:
        ...
        assert not self.ball_gaussians
        if self.gaussian_2d:
    ...
    def refinement_after(self, step: int, optimizer: torch.optim.Optimizer) -> None:
        ...
        if self.step <= self.ctrl_cfg.warmup_steps:
            return
        with torch.no_grad():
            # only do this if we are not in the freeze geometry phase
            if not self.freeze_geometry_flag:
                ...
                if do_densification:
                    ...
                    self._normals = Parameter(torch.cat([self._normals, split_normals, dup_normals], dim=0))
                    self._albedos = Parameter(torch.cat([self._albedos, split_albedos, dup_albedos], dim=0))
                    self._roughnesses = Parameter(torch.cat([self._roughnesses, split_roughnesses, dup_roughnesses], dim=0))
                    self._metallics = Parameter(torch.cat([self._metallics, split_metallics, dup_metallics], dim=0))
    ...
    def split_gaussians(self, split_mask: torch.Tensor, samps: int = 2) -> Tuple:
        ...
        return new_means, new_feature_dc, new_feature_rest, new_opacities, new_scales, new_quats, new_ids, new_normals, new_albedos, new_roughnesses, new_metallics
    ...
    def dup_gaussians(self, dup_mask: torch.Tensor) -> Tuple:
        ...
        return dup_means, dup_feature_dc, dup_feature_rest, dup_opacities, dup_scales, dup_quats, dup_ids, dup_normals, dup_albedos, dup_roughnesses, dup_metallics
    ...
    def transform_normals(self, normals: torch.Tensor) -> torch.Tensor:
        """
        transform the normals of instances to world space
        according to the pose at the current frame
        """
        ...
        global_rots_per_pts = global_rots_cur_frame[self.point_ids[..., 0]]
        
        # transform the normals to world space
        normals = self.normal_act(normals)
        normals = torch.bmm(
            global_rots_per_pts, normals.unsqueeze(-1)
        ).squeeze(-1)
    ...
    def get_gaussians(self, cam: dataclass_camera) -> Dict[str, torch.Tensor]:
        ...
        world_normals = self.transform_normals(self.get_normals)
        ...
        # normals_local_minscale = self.transform_normals(self.get_normals_minscale(self.get_scaling, self.get_quats)).detach()
    ...
    def remove_instances(self, remove_id_list: List[int]) -> None:
        ...
        for ins_ids in remove_id_list:
            ...
            self._albedos = Parameter(self._albedos[mask])
            self._roughnesses = Parameter(self._roughnesses[mask])
            self._metallics = Parameter(self._metallics[mask])
        ...
        for id in ids:
            if id not in gaussian_dict:
                instance_raw_dict = {
                    ...
                    "_albedos": self._albedos[self.point_ids[..., 0] == id],
                    "_roughnesses": self._roughnesses[self.point_ids[..., 0] == id],
                    "_metallics": self._metallics[self.point_ids[..., 0] == id],
                }
