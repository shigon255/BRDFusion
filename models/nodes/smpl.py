...
class SMPLNodes(RigidNodes):
    def __init__(
        ...
        **kwargs
        ...
        # NOTE: ball gaussian might not be capable for current pipeline
    ...
    def create_from_pcd(self, instance_pts_dict: Dict[str, torch.Tensor]) -> None:
        ...
        # TBD: add ctrl_cfg for normal freezing
        # self._normals = Parameter(torch.zeros(self.num_points, 3, device=self.device))
        ...
        self._normals = Parameter(torch.randn(self.num_points, 3, device=self.device))
        self._albedos = Parameter(torch.randn(self.num_points, 3, device=self.device))
        self._roughnesses = Parameter(torch.randn(self.num_points, 1, device=self.device))
        self._metallics = Parameter(torch.randn(self.num_points, 1, device=self.device))
    ...
    def transform_means_and_quats_and_normals(self, means: torch.Tensor, quats: torch.Tensor, normals: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        reshaped_normals = normals.reshape(self.num_instances, self.smpl_points_num, 3)
        deformed_normals = torch.einsum(
            "bnij,bnj->bni", R, reshaped_normals[instance_mask]         
        )  # [N, 6890, 3]
        normals_container = torch.zeros_like(reshaped_normals)
        normals_container.index_add_(0, instance_mask.nonzero().squeeze(), deformed_normals)
        normals_container = normals_container.reshape(-1, 3)
        
        return means_container, quats_container, normals_container
    ...
    def get_gaussians(self, cam: dataclass_camera) -> Dict[str, torch.Tensor]:
        ...
        if self.ball_gaussians:
            ...
            world_normals = self._normals
        else:
            # world_means, world_quats = self.transform_means_and_quats(self._means, self._quats)
            world_means, world_quats, world_normals = self.transform_means_and_quats_and_normals(self._means, self._quats, self.get_normals)
        ...
        # _, _, normals_local_minscale = self.transform_means_and_quats_and_normals(self._means, self._quats, self.get_normals_minscale(self.get_scaling, self.get_quats))
        # normals_local_minscale = normals_local_minscale.detach()
    ...
    def get_instance_activated_gs_dict(self, ins_id: int) -> Dict[str, torch.Tensor]:
        ...
        activated_local_normals = self.normal_act(self._normals[pts_mask])
        activated_albedos = self.get_albedos[pts_mask]
        activated_roughnesses = self.get_roughnesses[pts_mask]
        activated_metallics = self.get_metallics[pts_mask]
        gaussian_dict = {
            "means": local_means,
            ...
            "normals": activated_local_normals,
            "albedos": activated_albedos,
            "roughnesses": activated_roughnesses,
            "metallics": activated_metallics,
        }
        ...
        for id in ids:
            if id not in gaussian_dict:
                instance_raw_dict = {
                    ...
                    "_albedos:": self.get_albedos[self.point_ids[..., 0] == id],
                    "_roughnesses": self.get_roughnesses[self.point_ids[..., 0] == id],
                    "_metallics": self.get_metallics[self.point_ids[..., 0] == id],
    ...
    def remove_instances(self, remove_id_list: List[int]) -> None:
        ...
        """
        # Remove from large to small to avoid index-shift ambiguity.
        valid_ids = sorted(set(int(x) for x in remove_id_list), reverse=True)
        for ins_id in valid_ids:
            if ins_id < 0 or ins_id >= self.num_instances:
                continue

            pts_keep_mask = ~(self.point_ids[..., 0] == ins_id)
            self._means = Parameter(self._means[pts_keep_mask])
            self._scales = Parameter(self._scales[pts_keep_mask])
            self._quats = Parameter(self._quats[pts_keep_mask])
            self._features_dc = Parameter(self._features_dc[pts_keep_mask])
            self._features_rest = Parameter(self._features_rest[pts_keep_mask])
            self._opacities = Parameter(self._opacities[pts_keep_mask])
            self.point_ids = self.point_ids[pts_keep_mask]
            self._normals = Parameter(self._normals[pts_keep_mask])
            self._albedos = Parameter(self._albedos[pts_keep_mask])
            self._roughnesses = Parameter(self._roughnesses[pts_keep_mask])
            self._metallics = Parameter(self._metallics[pts_keep_mask])

            # Reindex surviving point instance ids to keep them in [0, num_instances-2].
            shift_mask = self.point_ids[..., 0] > ins_id
            self.point_ids[shift_mask, 0] -= 1

            ins_keep_mask = torch.ones(self.num_instances, device=self.device, dtype=torch.bool)
            ins_keep_mask[ins_id] = False
            self.instances_fv = self.instances_fv[:, ins_keep_mask]
            self.instances_trans = Parameter(self.instances_trans[:, ins_keep_mask])
            self.instances_quats = Parameter(self.instances_quats[:, ins_keep_mask])
            self.smpl_qauts = Parameter(self.smpl_qauts[:, ins_keep_mask])
            self.instances_size = self.instances_size[ins_keep_mask]

            # process_smpl_template
            self.template.remove_instance(ins_id)
