...
class DrivingDataset(SceneDataset):
    def __init__(
        ...
        if self.type == "self":
            ...
            cam_id = self.data_cfg.pixel_source.cameras[0]
            cam_name = DATASETS_CONFIG["self"][cam_id]["camera_name"]
            pose_path = os.path.join(self.data_path, f"{cam_name}_poses.txt")
            if not os.path.exists(pose_path):
                raise ValueError(
                    "Unable to determine total frames. Missing pose file: "
                    f"{pose_path}"
                )
            ...
            with open(pose_path, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    parts = line.split()
                    if len(parts) >= 8:
                        unique_frame_ids.add(int(parts[0]))
            total_frames = len(unique_frame_ids)
            if total_frames == 0:
                raise ValueError(f"No valid pose rows found in {pose_path}")
        ...
        assert self.pixel_source is not None, "Pixel source must be initialized"
        if self.data_cfg.lidar_source.load_lidar:
            assert self.lidar_source is not None, "Lidar source must be initialized when load_lidar is True"
        if (
            self.lidar_source is not None
            and not self.data_cfg.pixel_source.get("load_only_calibrations", False)
        ):
    ...
    def get_lidar_samples(
        ...
        if self.lidar_source is None:
            logger.warning("No lidar source available, returning empty lidar samples.")
            empty_pts = torch.empty(0, 3, device=device)
            empty_color = torch.empty(0, 3, device=device) if return_color else None
            empty_time = torch.empty(0, device=device) if return_normalized_time else None
            return empty_pts, empty_color, empty_time
    ...
    def get_init_objects(
        ...
        """
        if self.lidar_source is None or self.instance_num == 0:
            logger.warning("No lidar or instances available, returning empty init objects.")
            return {}
        if self.type == "KITTI":
        ...
        for ins_id in instance_dict:
            # filter out the instance with no points
            # to avoid CUDA invalid configuration error
            instance_dict[ins_id]["pts"] = [pts for pts in instance_dict[ins_id]["pts"] if pts.shape[0] > 0]
            instance_dict[ins_id]["colors"] = [pts for pts in instance_dict[ins_id]["colors"] if pts.shape[0] > 0]
            try:
                if len(instance_dict[ins_id]["pts"]) == 0:
                    instance_dict[ins_id]["pts"] = torch.empty(0, 3).to(self.device)
                    instance_dict[ins_id]["colors"] = torch.empty(0, 3).to(self.device)
                else:
                ...
                print(f"Error in instance {ins_id}: {e}")
    ...
    def get_init_smpl_objects(self, only_moving: bool = False, traj_length_thres: float = 0.5):
        if self.lidar_source is None:
            logger.warning("No lidar source available, returning empty SMPL init objects.")
            return {}
        if not hasattr(self.pixel_source, "smpl_human_all") or len(self.pixel_source.smpl_human_all) == 0:
            logger.warning("No SMPL data available, returning empty SMPL init objects.")
            return {}
        instance_dict = {}
        """
        instance_dict = {
        ...
        for ins_id in range(self.instance_num):
            ...
            if true_id in self.pixel_source.smpl_human_all.keys():
                ...
                # to avoid CUDA invalid configuration error
                try:
                    collected_lidar_pts = [pts for pts in collected_lidar_pts if pts.shape[0] > 0]
                    collected_lidar_colors = [pts for pts in collected_lidar_colors if pts.shape[0] > 0]
                    if len(collected_lidar_pts) == 0:
                        pts = torch.empty(0, 3).to(self.device)
                        colors = torch.empty(0, 3).to(self.device)
                    else:
