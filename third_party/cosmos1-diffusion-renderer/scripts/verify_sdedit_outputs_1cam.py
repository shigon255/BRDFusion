#!/usr/bin/env python3
"""Verify single-camera SDEdit pipeline outputs against expected structure and frame counts."""
...
def verify_inverse(args: argparse.Namespace) -> int:
    ...
    if failures:
        print("[verify-sdedit-1cam] FAILED")
        for item in failures:
            print(f" - {item}")
        return 1

    print("[verify-sdedit-1cam] OK (inverse)")
    return 0
...
def verify_forward(args: argparse.Namespace) -> int:
    ...
    for s in args.strengths:
        ...
        for output_name in FORWARD_OUTPUTS:
            path = strength_root / "videos" / str(CAM_ID) / f"{output_name}.mp4"
            if not path.exists():
                failures.append(f"missing file: {path}")
                continue
            frames = count_video_frames(path)
    ...
    print("[verify-sdedit-1cam] OK (forward)")
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(description="Verify single-camera SDEdit output contracts")
