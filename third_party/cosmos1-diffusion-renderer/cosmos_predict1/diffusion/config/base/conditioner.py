...
from cosmos_predict1.diffusion.conditioner import (
    ...
    VideoDiffusionRendererConditioner,
)
...
@attrs.define(slots=False)
class ContextConditionConfig:
    """
    Remap the key from the input dictionary to the output dictionary. For `context index condition`.
    """

    obj: LazyDict = L(ReMapkey)(output_key="context_index", dtype="long")
    dropout_rate: float = 0.0
    input_key: str = "context_index"

VideoDiffusionRendererConditionerConfig: LazyDict = L(VideoDiffusionRendererConditioner)(
    text=TextConfig(),
    fps=FPSConfig(),
    num_frames=NumFramesConfig(),
    image_size=ImageSizeConfig(),
    ...
    context_index=ContextConditionConfig(),
)
