...
def save_image_or_video(video_save_path, video, H=704, W=1280, fps=24, video_save_quality=5):
    """Save video frames to file. If video has only 1 frame, saves as JPG instead.
    Args:
        grid (np.ndarray): Video frames array [T,H,W,C]
        fps (int): Frames per second
        H (int): Frame height
        W (int): Frame width
        video_save_quality (int): Video encoding quality (0-10) or JPEG quality (0-100)
        video_save_path (str): Output file path. Extension will be changed to .jpg for single frames
    """
    if len(video) == 1:  # Single frame case
        frame = video[0]
