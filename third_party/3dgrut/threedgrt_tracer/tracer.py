...
class Tracer:
    ...
    def builc_acc_neilf(self, gaussians, rebuild=True):
        with torch.cuda.nvtx.range(f"build-bvh-full-build-{rebuild}"):
            allow_bvh_update = (
                self.conf.render.max_consecutive_bvh_update > 1
            ) and not self.conf.render.particle_kernel_density_clamping
            ...
            self.tracer_wrapper.build_bvh(
                gaussians.get_xyz.view(-1, 3).contiguous(),
                gaussians.get_rotation.view(-1, 4).contiguous(),
                gaussians.get_scaling.view(-1, 3).contiguous(),
                gaussians.get_opacity.view(-1, 1).contiguous(),
                rebuild_bvh,
                allow_bvh_update,
            )
            self.num_update_bvh = 0 if rebuild_bvh else self.num_update_bvh + 1
    ...
    def render_neilf(self, gaussians, gpu_batch: Batch, train=False, frame_id=0, opacity_mask=None):
        ...
        with torch.cuda.nvtx.range(f"model.forward({num_gaussians} gaussians)"):
            ...
            opacities = gaussians.get_opacity*opacity_mask if opacity_mask is not None else gaussians.get_opacity
            features = gaussians.get_shs
            features_dc = features[:, 0, :]
            features_rest = features[:, 1:, :].reshape(num_gaussians, -1)
            features = torch.cat([features_dc, features_rest], dim=1)
            (pred_rgb, pred_opacity, pred_dist, pred_normals, hits_count, mog_visibility) = Tracer._Autograd.apply(
                self.tracer_wrapper,
                frame_id,
                ...
                gpu_batch.rays_ori.contiguous(),
                gpu_batch.rays_dir.contiguous(),
                gaussians.get_xyz.contiguous(),
                gaussians.get_rotation.contiguous(),
                gaussians.get_scaling.contiguous(),
                opacities.contiguous(),
                features.contiguous(),
                Tracer.RenderOpts.DEFAULT,
                3, # gaussians.n_active_features, hardcode for now
