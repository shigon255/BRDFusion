...
from threedgrut.datasets import NeRFDataset, ColmapDataset, ScannetppDataset, CamTrajDataset
...
def create_test_dataloader_outer(conf):
        ...
        elif conf.dataset.type == "scannetpp":
            dataset = ScannetppDataset(conf.path, split="val")
        elif conf.dataset.type == "camtraj":
            dataset = CamTrajDataset(conf.path) # no split
        else:
            raise ValueError(
...
def init_scene_extents(train_dataset: BoundedMultiViewDataset) -> None:
    ...
    return scene_extent, scene_bbox

class Renderer:
    def __init__(
        self, model, conf, global_step, out_dir, path="", save_gt=True, writer=None, compute_extra_metrics=True, dataset=None, dataloader=None
    ) -> None:
    ...
    @classmethod
    def from_ply(
        cls, ply_path, out_dir, conf, path="", save_gt=False, writer=None, model=None, computes_extra_metrics=False
    ):
        """Loads checkpoint for test path.
        If path is stated, it will override the test path in checkpoint.
        If model is None, it will be loaded base on the
        """
        assert conf is not None, "config must be provided"

        # global_step = checkpoint["global_step"]
        global_step = 0 # dummy value
        ...
        # create dataset in advance to get scene extent
        dataset, dataloader = create_test_dataloader_outer(conf)
        scene_extent, _ = init_scene_extents(dataset)
        ...
        if model is None:
            ...
            # Initialize the parameters from ply
            model.init_from_ply(ply_path)
        # import pdb; pdb.set_trace()
        from time import time
        t = time()
        model.build_acc()
        print(f"Time to build acc: {time()-t}")
    ...
    def render_all(self):
        ...
        for iteration, batch in enumerate(self.dataloader):
            ...
            logger.info(f"Frame {iteration}, Inference time: {inference_time[-1]} ms")
        ...
        logger.info(f"Mean inference time: {mean_inference_time} ms/frame")
