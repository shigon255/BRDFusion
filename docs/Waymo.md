...
python datasets/waymo/waymo_download.py \
    ...
    --scene_ids 3 19 114 172 703
```

You can also provide a split file (e.g. `data/waymo_brdfusion_list.txt`) to download a batch of scenes at once:

```shell
GSUTIL=/path/to/gsutil \
python datasets/waymo/waymo_download.py \
    --target_dir ./data/waymo/raw \
    --split_file data/waymo_brdfusion_list.txt
```
...
```shell
# run in a separated env is recommended
...
python -m datasets.tools.extract_masks \
