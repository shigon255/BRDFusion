...
SPIRAL_TRAJECTORY_MODE="${SPIRAL_TRAJECTORY_MODE:-circle}"
SPIRAL_CIRCLE_START_SIDE="${SPIRAL_CIRCLE_START_SIDE:-top}"
SPIRAL_CIRCLE_RADIUS_RIGHT_M="${SPIRAL_CIRCLE_RADIUS_RIGHT_M:-${SPIRAL_RADIUS_M}}"
SPIRAL_CIRCLE_RADIUS_LEFT_M="${SPIRAL_CIRCLE_RADIUS_LEFT_M:-${SPIRAL_RADIUS_M}}"
SPIRAL_CIRCLE_RADIUS_UP_M="${SPIRAL_CIRCLE_RADIUS_UP_M:-${SPIRAL_RADIUS_M}}"
SPIRAL_CIRCLE_RADIUS_DOWN_M="${SPIRAL_CIRCLE_RADIUS_DOWN_M:-${SPIRAL_RADIUS_M}}"
SPIRAL_VERTICAL_AMPLITUDE_M="${SPIRAL_VERTICAL_AMPLITUDE_M:-0.3}"
SPIRAL_DOWN_BIAS_M="${SPIRAL_DOWN_BIAS_M:-0.0}"
SPIRAL_PITCH_MODE="${SPIRAL_PITCH_MODE:-look_at}"
...
INNERPY
    ...
    INSERT_ANCHOR_TIMESTEP="${INSERT_ANCHOR_TIMESTEP:-${ANCHOR_TIMESTEP:-${START:-0}}}"
    ...
    INSERT_PITCH_UP_DEG="${INSERT_PITCH_UP_DEG:-${PITCH_UP_DEG:-0.0}}"
    INSERT_ROLL_LEFT_DEG="${INSERT_ROLL_LEFT_DEG:-${ROLL_LEFT_DEG:-0.0}}"
    ...
    EXTRA_EVAL_ARGS+=(
      ...
      --insert_pitch_up_deg "${INSERT_PITCH_UP_DEG}"
      --insert_roll_left_deg "${INSERT_ROLL_LEFT_DEG}"
...
if [[ -n "${ASSET_SPEC}" ]]; then
  ...
  CLASSES="${CLASSES:-}"
  ANCHOR_TIMESTEP="${ANCHOR_TIMESTEP:-${START:-0}}"
...
INSERT_MOTION_START_TIMESTEP="${INSERT_MOTION_START_TIMESTEP:-${MOTION_START_TIMESTEP:-${START:-}}}"
CALIBRATE_ENVMAP_TIMESTEP="${CALIBRATE_ENVMAP_TIMESTEP:-0}"
...
if [[ -n "${INSERT_MOTION_START_TIMESTEP}" ]]; then
  cmd+=(--insert_motion_start_timestep "${INSERT_MOTION_START_TIMESTEP}")
fi
