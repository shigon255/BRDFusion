...
def color_map(t):
    # t is a torch tensor with shape (N,)
    # this funciton maps t from [0, 1] to RGB color using a colormap
    # map flat rate scalar to RGB using an HSV->RGB colormap (blue -> red)
        
    t = t.reshape(-1)  # (N,)
    t = torch.clamp(t, 0.0, 1.0)
        
    # hue from blue (0.66) to red (0.0)
    h = (1.0 - t) * 0.66
    f = (h * 6.0) % 1.0
    i = (h * 6.0).floor().to(torch.long) % 6

    q = 1.0 - f
    one = torch.ones_like(f)
    zero = torch.zeros_like(f)

    r = torch.where(i == 0, one, zero)
    g = torch.where(i == 0, f, zero)
    b = torch.where(i == 0, zero, zero)

    r = torch.where(i == 1, q, r)
    g = torch.where(i == 1, one, g)
    b = torch.where(i == 1, zero, b)

    r = torch.where(i == 2, zero, r)
    g = torch.where(i == 2, one, g)
    b = torch.where(i == 2, f, b)

    r = torch.where(i == 3, zero, r)
    g = torch.where(i == 3, q, g)
    b = torch.where(i == 3, one, b)

    r = torch.where(i == 4, f, r)
    g = torch.where(i == 4, zero, g)
    b = torch.where(i == 4, one, b)

    r = torch.where(i == 5, one, r)
    g = torch.where(i == 5, zero, g)
...
def vis_surface_normal(normal: torch.tensor, mask: torch.tensor=None) -> np.array:
    ...
    normal_vis = (n_img_norm.copy() + 1.0) / 2.0
    if mask is not None:
...
def layout_self(
    ...
    """Combine cameras into a single-row layout in a left-to-right order."""
    desired_order = ["Cam_Left", "Camera_Center", "Cam_Right"]
    name_to_img = {name: img for name, img in zip(cam_names, imgs)}
    ordered_imgs = [name_to_img[name] for name in desired_order if name in name_to_img]
    if not ordered_imgs:
        ordered_imgs = imgs
    return np.concatenate(ordered_imgs, axis=1)
...
def depth_to_grayscale(depth_map, opacity_mask=None, lo=None, hi=None, percentile=99.0,
                       mask_threshold=0.05):
    """
    Normalize depth linearly to [0, 1] and create 3-channel grayscale image.
    
    Args:
        depth_map: (H, W) depth values
        opacity_mask: (H, W) optional weight/opacity mask
        lo: minimum depth value (if None, auto-compute from percentile)
        hi: maximum depth value (if None, auto-compute from percentile)
        percentile: percentile for auto-ranging
    
    Returns:
        (H, W, 3) grayscale image in [0, 1]
    """
    if opacity_mask is None:
        opacity_mask = np.ones_like(depth_map)

    # Auto-compute bounds using weighted percentiles if not provided
    if lo is None or hi is None:
        from utils.visualization import weighted_percentile
        lo_auto, hi_auto = weighted_percentile(
            depth_map, opacity_mask, [50 - percentile / 2, 50 + percentile / 2]
        )
        lo = lo if lo is not None else lo_auto
        hi = hi if hi is not None else hi_auto

    # Linear normalization
    depth_normalized = np.clip((depth_map - lo) / (hi - lo + 1e-8), 0, 1)

    # Apply opacity masking - low weight regions (sky) get max depth (1.0)
    # depth_normalized = depth_normalized * opacity_mask + (1 - opacity_mask)

    # set the depth with opacity below threshold to 1.0 (max depth)
    depth_normalized[opacity_mask < mask_threshold] = 1.0

    # Expand to 3 channels for grayscale
    depth_grayscale = np.stack([depth_normalized] * 3, axis=-1)
